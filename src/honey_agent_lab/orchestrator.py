...
def run_scenario(name: str, ledger: AppendOnlyAuditLedger | None = None) -> SimulationResult:
    scenario = get_scenario(name)
    active_ledger = ledger or AppendOnlyAuditLedger()
    bus = SimulatedAgentBus()
    honey = HoneyAgent(ledger=active_ledger)
    ...
    active_ledger.record(
        "scenario_completed",
        {
            ...
            "final_action": results[-1].decision.action.value if results else "none",
        },
    )
